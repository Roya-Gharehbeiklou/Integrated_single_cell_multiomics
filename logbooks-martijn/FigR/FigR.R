...
cisCorr <- read.csv('logbooks/FigR_plots/ciscorr.csv', sep='\t')
...
dorcGenes <- dorcJPlot(dorcTab = cisCorr.filt,
                         ...
                         force=2)
ggsave('logbooks/FigR_plots/DORCS.png')
...
# DORC score for Pxn
dorcg <- plotMarker2D(umap.d,dorcMat.s,markers = c("Pxn"),maxCutoff = "q0.99",colorPalette = "brewer_heat") + ggtitle("Dlx3 DORC")

ggsave('logbooks/FigR_plots/dorc-Pnx.png', dorcg)

# RNA for Dlx3
rnag <- plotMarker2D(umap.d,RNAmat.s,markers = c("Pxn"),maxCutoff = "q0.99",colorPalette = "brewer_purple") + ggtitle("Dlx3 RNA")

ggsave('logbooks/FigR_plots/rna-Pnx.png', rnag)
...
runFigRGRN <- function(ATAC.se, # SE of scATAC peak counts. Needed for chromVAR bg peaks etc.
  ...
  packagePath <- find.package("FigR", 'R_libs', quiet = TRUE)
...
figR.d %>% 
  ...
  ggsave('logbooks/FigR_plots/tf-dorc-associations.png')
head(figR.d)
ranks <- rankDrivers(figR.d,rankBy = "meanScore",interactive = FALSE)
ggsave('logbooks/FigR_plots/ranking-drivers-tf.png', ranks)
...
ggsave('logbooks/FigR_plots/plot-drivers.png', plt.drivers)
...
heatmp.plt
ggsave('logbooks/FigR_plots/heatmap.png', heatmp.plt)
